...
| **groceries** | `/groceries:order-from-anylist`, `/groceries:add-to-mandje` | | | macOS | Grocery automation built on the `groceries` Ruby CLI in the kassa monorepo (Picnic plus AnyList; credentials in the macOS Keychain). `order-from-anylist` runs the LLM-assisted triage of an AnyList list into the Picnic mandje with operator approval; `add-to-mandje` adds a single product without touching AnyList. Commands run as `groceries-cli/bin/groceries` from the kassa repo root. |

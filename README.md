...
| **anger-management** | `/fuck`, `/shit`, `/crap`, `/wtf`, `/bullshit`, `/damn`, `/anger-management`, `/anger-management:repair` | | | | Curse at your coding agent now, fix the real problem later. Each curse command captures one cheap friction line to a global log (`~/.claude/var/leclause/anger-management/friction.jsonl`, note fed on stdin via a quoted heredoc) and gets out of the way. A capture arms a single background investigation that, 22m22s later, has a headless `claude` agent read the open captures plus repair history plus recent transcripts and write a diagnosis; a check-in cron taps you when it is ready. `/anger-management:repair` opens with a blunt go/no-go (nothing / not-enough-signal / fix), and a real recurring problem is routed to `/self-improvement`, often by loosening a rule that overcorrected rather than adding one. The pile is global and accumulates across sessions and repos, so deferring sharpens the diagnosis. Grounded in Anil Seth's "controlled hallucination", constructed emotion, the catharsis-is-a-myth finding, and signal-vs-pattern anger. Mirrors your own log (no profiling), tongue-in-cheek. State is stored locally; the optional background investigation uses the configured runner. Vocabulary avoids slurs and identity-targeted abuse. See [anger-management](packages/anger-management/README.md). |

...
4. Add a `whats-new` skill that calls the helper with `--force` so the user can re-read the latest section on demand. See `packages/gitgit/skills/whats-new/SKILL.md`.
...
Volgordecriterium: the user MUST NOT be surprised by breaking changes. Phrase breaking entries unambiguously and at the top of the section. Non-breaking additions can be a soft nudge; never gate the user's actual work on acknowledgement.

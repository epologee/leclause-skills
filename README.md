...
| **gitgit** | `/gitgit:commit-all-the-things`, `/gitgit:commit-snipe`, `/gitgit:rebase-latest-default`, `/gitgit:merge-to-default` | | | | Bundle of git-write skills. `commit-all-the-things` groups every uncommitted change into logical commits. `commit-snipe` stages only what belongs to the current task. `rebase-latest-default` rebases the current branch on the freshest default branch (local or `origin/<default>`). `merge-to-default` lands the current branch on the project's default with a github-style `--no-ff` merge commit, rebases on conflict, and no-ops with a TUI warning when invoked on the default branch itself. |

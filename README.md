...
| **gitgit** | `/gitgit:commit-all-the-things`, `/gitgit:commit-snipe`, `/gitgit:rebase-latest-default`, `/gitgit:merge-to-default`, `/gitgit:commit-discipline`, `/gitgit:install-hooks`, `/gitgit:run-spec`, `/gitgit:saw-red` | ✅ | ✅ | | Bundle of git-write skills plus commit-discipline hooks. `commit-all-the-things` groups every uncommitted change into logical commits. `commit-snipe` stages only what belongs to the current task and auto-activates on the word "snipe". `rebase-latest-default` rebases the current branch on the freshest default branch (local or `origin/<default>`). `merge-to-default` lands the current branch on the project's default with a github-style `--no-ff` merge commit, rebases on conflict, deletes the local source branch after the merge is confirmed, and no-ops with a TUI warning when invoked on the default branch itself. `commit-discipline` (slices 1-9 complete) ships PreToolUse:Bash and git-native hooks that enforce a structured commit body schema: subject + WHY paragraph + Slice / Tests / Red-then-green trailers parsed via `git interpret-trailers`, with opt-out enum tokens (docs-only, config-only, chore-deps, wip, merge, revert) and an audit-shadow-log helper. `install-hooks` copies the git-native `commit-msg`, `post-commit`, and `pre-push` scripts into the current repo's `.git/hooks/` (or `core.hooksPath` when set) so commits and pushes made outside Claude Code still get validated; `--force` to overwrite existing hooks, `--dry-run` to preview. The `pre-push` hook (and a parallel PreToolUse:Bash guard on `git push`) enforce the slice-7 wip-gate: any commit in the push range with a `Slice: wip` trailer blocks the push unless explicitly bypassed via `GITGIT_ALLOW_WIP_PUSH=1` or the magic-comment `# allow-wip-push`; bypasses are appended to `~/.claude/var/gitgit-wip-pushes.log`. The `prepare-commit-msg` hook (installed alongside `commit-msg`) pre-fills the editor with a WHY-comment block and an auto-detected body schema on every normal interactive commit: staged paths are classified into layer tokens (frontend / backend / spec / migration / config / docs / other), producing a suggested Slice line and a suggested Tests list seeded from spec-pattern paths; merge, squash, amend, and `-m` commits are skipped so the hook is invisible outside the interactive editor workflow. Also bundles the absorbed user-level guards `commit-trailers.sh` (blocks `Co-Authored-By:` trailers with an `@anthropic.com` email; bypass via `GITGIT_ALLOW_AI_COAUTHOR=1`) and `git-dash-c.sh` (blocks `git -C <dir>` so Claude Code's prefix-based permission matching stays clean). `run-spec` runs a single test/spec file through the project's auto-detected runner (RSpec, Jest, Vitest, Go test, pytest), records the exit code in the test-runner cache (`~/.claude/var/gitgit-test-runs.log`, override via `GITGIT_TEST_CACHE`), and prints a PASS/FAIL summary; when `GITGIT_TEST_CACHE_REQUIRED=1`, `commit-body.sh` rejects commits whose `Tests:` trailer paths have no recent green cache entry. `saw-red` writes a RED (failing) cache entry for a spec path you observed failing outside `/gitgit:run-spec` (IDE run, terminal, CI), enabling the `Red-then-green: yes` trailer validation to pass once a subsequent green run is recorded. |

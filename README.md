...
| **dont-do-that** | `/do-that` | | ✅ | | Eleven guardrail hooks (one dispatcher, uniform `[dont-do-that/<code>]` messages) that push back on common AI reflexes: shifting blame, stopping prematurely, delegating verification, offering a recipe instead of executing it (`do-that` guard), asking for confirmation when none was needed, sloppy commit-message format, and em-dashes in prose. Plus one user-invocable skill: `/do-that` is the operator's one-keystroke correction when Claude proposed an action instead of running it: it tells Claude to execute the proposal from the previous turn. See [dont-do-that](packages/dont-do-that/README.md). |

...
| **anger-management** | `/fuck`, `/shit`, `/crap`, `/wtf`, `/bullshit`, `/damn`, `/anger-management` | | | | A swear jar that pays you back in workflow improvements. When a session grates and you have no appetite to be constructive right then, vent: each venting command appends one cheap friction "paper-cut" line to `~/.claude/var/leclause/anger-management/friction.jsonl` (the pointer is fed on stdin via a quoted heredoc, so it is never interpreted by the shell) and resumes work, no fix demanded in the moment. `/anger-management` reads the pile back, ranks recurring frictions, and offers to route the worst to `/self-improvement`. Modelled on Stripe's "bad day button" and friction logging: the vent is the zero-effort capture, the constructive pass happens later on your terms. The vocabulary is deliberately gender-neutral and non-discriminatory (scatological and generic-sexual words). The venting commands never auto-activate; they run only when you type them. See [anger-management](packages/anger-management/README.md). |

...
| **how-plugins-work** | `/how-plugins-work`, `/how-plugins-work:test-before-push` | ✅ | | | Living document explaining how Claude Code plugin naming, skill resolution, cross-agent skill sync, and the plugin:skill invocation pattern work. The cross-agent section keeps shared `SKILL.md` files, agent-specific manifests, marketplace indexes, and runtime caches in separate source/adapter/cache roles. The `test-before-push` sub-skill encodes the canonical procedure for pointing a marketplace alias at the local working copy so you can test a plugin change in a fresh Claude session without pushing first. |
| **inspire** | `/inspire` | ✅ | | | Online research workflow for unfamiliar topics, design decisions, and evaluating approaches. |
| **intervision** | `/intervision:second-opinion` | | | | Peer consultation for coding agents. Brings another vendor's coding agent (Codex) in as a peer of equal standing to review work just done or just discussed, the opposite of supervision from above. `/intervision:second-opinion` hands the diff (`codex exec review --uncommitted` / `--base` / `--commit`) or a just-discussed design (read-only `codex exec`) to Codex, surfaces its independent read, and goes back and forth via `codex exec resume`. Codex's findings come home through three fates (fix, cost-value-skip, reject-as-hollow) and the remaining disagreements are surfaced to the operator. Requires the `codex` CLI installed and logged in; the skill preflights and stops gracefully when no peer is available. |

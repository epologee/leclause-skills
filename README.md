...
| **intervision** | `/intervision:confer` | | | | Peer consultation for coding agents. Brings another vendor's coding agent (Codex) in as a peer of equal standing to review work just done or just discussed, the opposite of supervision from above. `/intervision:confer` hands the diff (`codex exec review --uncommitted` / `--base` / `--commit`) or a just-discussed design (read-only `codex exec`) to Codex, surfaces its independent read, and confers back and forth via `codex exec resume`. Codex's findings come home through three fates (fix, cost-value-skip, reject-as-hollow) and the remaining disagreements are surfaced to the operator. Requires the `codex` CLI installed and logged in; the skill preflights and stops gracefully when no peer is available. |

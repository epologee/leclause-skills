...
**Worked dry-run of the new flow against the Portier scope.** A calling session in the same Portier scope today walks the formulation prompts and writes the canonical-channel-bypass entry by following the prompt's structure literally:

| Prompt input | What the session writes | Sites a `rg` against the signature would hit |
|--------------|-------------------------|---------------------------------------------|
| Canonical-channel bypass: which domain values have a canonical formatter, and where are they rendered/spoken/logged/interpolated *outside* that formatter? | Pattern `plate-outside-formatter`: `LicensePlateFormatter` is the canonical channel for Dutch plate text. Audit every site where a `Plate` value is interpolated into a user-facing string without going through this formatter (Views, dialogs, accessibility labels, AppIntent display labels, push notification bodies, logs). Signatures: <code>rg -n '\\\\(plate\\|kenteken)\\b' Sources/</code> for raw interpolation; <code>rg -nF 'LicensePlateFormatter' Sources/</code> for canonical call-sites, take the complement. | `DutchPlateBadge.swift` (the View case the original audit caught); `RegisterVisitorConfirmDialog.swift` (the confirmationDialog message "NK341KZ aanmelden vandaag?"); `PermitRowView.swift` accessibility label; `RegisterPlateAppIntent.swift` display label. |

The four sites the original audit missed appear in the complement of `rg -nF 'LicensePlateFormatter' Sources/` because each of them interpolates the plate as a raw `String(describing:)` or `\\(plate.raw)`. The Sweep step would then run that signature, verify each site with its own per-finding verifier_command, and surface all four. Step 2.5 reduces the residual risk further: if the calling session writes a narrower pattern (only `Text(...)` modifiers, repeating the original miss), the contrarian subagent reading the same scope flags "the entry as written misses `.confirmationDialog`, `accessibilityLabel`, and AppIntent display labels carrying the same value; broaden the signature or split into a second entry." The four sites still land.

The dry-run is not a guarantee. A session that writes a deliberately narrow entry, refuses to broaden it after the contrarian, and runs sweep anyway can still ship a Portier-shaped miss. The discipline does what discipline does: it makes the failure mode visible (the contrarian's flag becomes part of the artefact's `## Detection method chosen` paragraph), and the visible failure is a finding the next audit can act on. A canned template offered no visibility.

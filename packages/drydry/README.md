...
**The eight formulation prompts are themselves a taxonomy, deliberately so.** Drydry does not eliminate canning; it relocates canning to a more general level. The previous mechanism shipped per-domain templates (the SwiftUI seed knew View-rendering, the Rails seed knew service objects, and so on), and a session was funneled into whichever template matched the domain hint. The new mechanism ships per-discipline lenses (canonical-channel bypass, predicate pairs, framework-seam boilerplate, and the rest), and a session is funneled into asking the same eight questions of any scope. That is a generality improvement, not a canning escape. The honesty matters because the value of the redesign rests on the claim that questions-about-any-codebase outperform answers-for-a-named-domain; the prompts are still the plugin's prior knowledge, just held at one floor up. The Portier-shaped failure mode survives when the eight lenses are insufficient for a specific codebase's idiom; the off-template-patterns prompt and step 2.5 exist as the explicit safety net for that case.

...
2. **Formulate the checklist (the calling session does this work).** Drydry does not hand the calling session a canned list of patterns. Instead the orchestrator instructs the calling session to read the scope and formulate a six-to-ten item duplication checklist by walking these formulation prompts against the actual codebase:
   ...
   - **User-facing copy with structural variants.** Which user-visible strings (confirmation copy, error messages, empty-state text) appear in two or more variants that solve the same UX need? Drift here erodes design-system consistency.
   ...
   - **Off-template patterns specific to this project.** What patterns does *this* codebase have that no general checklist would name? A custom routing convention, a homegrown event bus, a specific data-shaping idiom. The calling session sees these because it reads the code; the plugin cannot.

...
5. **Apply Chapter 7 (contrarian second-pass on rejects).** For every finding the sub-skill classified as needs-design or by-design (effectively a "this is not a real drift" verdict), spawn a second Sonnet subagent with a contrarian brief ("is this rejection hollow?"). A reject is final only if the contrarian independently confirms it as hollow.
...
- **Out-of-process follow-ups.** When a finding's convergence has to happen out-of-process (a separate session, a separate mission), the artefact currently logs it under `## Side quests` without a structured handoff format. Decide whether to formalise the handoff once a real case appears.

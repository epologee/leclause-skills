...
3. **What "finished" looks like.** One sentence describing the stop state, phrased so the operator can audit it against the Dispatch. The framing of this sentence is the diagnostic: a narrow technical state ("CI is green", "tests pass", "the endpoint returns 200") describes a different destination from a deliverable state ("the branch is ready for its next step", "the page is ready for users", "the release is shippable"). Narrow framings silently exclude inherited work on the deliverable; deliverable framings include it. Narrow framings are correct only when the Dispatch itself names a narrow technical state as the destination ("get CI green on branch X"); otherwise the rover defaults to a deliverable framing that matches the Dispatch's destination, not the rover's own commits. If the sentence cannot be written without hedging ("mostly", "except for the pre-existing X"), the understanding is wrong and the rover re-reads the Dispatch before continuing.

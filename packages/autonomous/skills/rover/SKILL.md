...
Note that setup step 1 (CronCreate) fires before prelaunch closes. It is not an operator-visible action and does not consume the window; the no-git-repo question in step 2 is still prelaunch.
...
1. Invoke `cron` via the Skill tool to `CronCreate`. Pass `.autonomous/<NAME>.md` as the loop-file path (derive `<NAME>` from the Dispatch the same way step 5 does: ALL-CAPS, hyphens, no spaces, goal not mechanism; no tool call needed for this, it comes out of reading the Dispatch text). The loop file does not have to exist yet; the cron prompt's Read is a no-op when the file is missing, and the next tick picks up once the file lands. Hold the returned job id in-session to write into the loop file in step 5. Cron goes first because setup has several generation-horizon hazards (the big template write in step 5 is the worst), and the cron has to be the safety net during those hazards, not a consequence of surviving them. If the prelaunch question in step 2 ends the mission (operator refuses the no-git-repo proposal), invoke `cron` again with `CronDelete` on the returned id before stopping so the cron does not outlive the abort.
2. Create the mission branch. Pick a kebab-case name from the loop-file name, no slashes, no prefixes, no rover or space-mission words (`fix-stale-cache`, `build-auth-page`, `investigate-slow-queries`), and run `git checkout -b <name>` directly from the current HEAD. **Do not switch to the default branch first.** The operator picked the starting point when they dispatched, and rewinding to the default branch silently abandons whatever feature work HEAD was sitting on; the operator does this on purpose to diverge from a parent branch that will later be merged-then-rebased. Any uncommitted changes in the working tree ride along onto the new branch, which is fine; step 3 handles them. Record the branch name in the loop file's `branch:` field in step 5. When HEAD is not on the default branch at rover start, do two things and only those two. First, print one visible line to the transcript with the literal branch name and short SHA so the operator can grep it later: `⚠️ Starting from non-default branch <CURRENT> at <SHORT_SHA>; mission branch builds on this work and will need a rebase-on-default once <CURRENT> lands.` Second, write the same line into step 5's Log block prefixed with `[HH:MM]` before the first SURVEY tick. That is the whole ceremony, no "are we on the right branch" question to the operator and no rewind to default. One case the rover cannot decide alone: no `.git` in cwd (or a repo with zero commits). Then pull step 5 forward as a stub and open a prelaunch question asking whether to `git init -b main` or set it up differently; wait for the answer or the five-minute fuse.
3. Commit any leftover working tree changes on the mission branch. If `git status --short` is empty, skip. Otherwise invoke `commit-all-the-things` via the Skill tool (sister plugin in the leclause-skills marketplace) to group the leftovers into logical commits with descriptive messages in the project's commit style, and log one line: `[HH:MM] ⚠️ Leftovers in the working tree, committed on mission branch: <one-line summary>.` That is the whole ceremony.
...
**Foresight (before DRIVE).** Before transitioning to DRIVE, the rover writes a `## Foresight` block into the loop file listing 3 to 5 concerns a human or bot reviewer would raise about the planned work. Each entry is tagged with one of three categories and gets one action. The verb "anticipate" is load-bearing: the rover is not writing the final review, it is predicting what reviewers will see and pre-deciding which predictions warrant work.

| Category   | Meaning                                                                                                                                                              | Action                                                                                                                          |
|------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------|
| core       | Hits the deliverable itself: the system would be wrong or incomplete without addressing this.                                                                        | Address before DRIVE. Fold into the Plan and Done criteria.                                                                     |
...
| noise      | Concern a reviewer might raise but that is outside this deliverable's scope.                                                                                         | Accept explicitly, one-line rationale in the Foresight block. No code action.                                                   |

The Foresight is a scope-discipline tool, not a TODO list. "Address everything" is not one of the actions. The `noise` category is the anti-creep escape: it is legitimate, and the rover takes responsibility for declining it on purpose rather than treating every reviewer concern as in-scope. Hard cap: max 5 entries. If the rover sees more, the Plan is too broad and should be narrowed.

The categorisation is Plan-time prediction, not finding-disposition. Compare against "Three fates" above: Three fates classifies findings AFTER they surface during SURVEY or INSPECT (fix, cost-value-skip, reject-as-non-issue). Foresight predicts the findings BEFORE they surface and pre-classifies which ones the work itself will address. The two rubrics run at different moments and serve different roles; do not collapse them into one.

The Foresight is operator-visible. The rover does not ask the operator to confirm it (the rover is autonomous), but the operator reads it later as the contract that pride and gurus are checked against at INSPECT.
...
Invoke `cron` via the Skill tool as setup step 1, before anything else. That skill's setup flow runs `CronCreate`, returns the job id for the rover to embed when it writes the loop file in step 5, and sets the initial interval to `* * * * *`.
...
The mission branch is created during setup (see step 2). The rover stays on that branch for the whole traverse, commits logical steps onto it, and never rebases or force-pushes it. Dropping the branch is how the operator reverses the mission, so the branch has to absorb every commit intact.

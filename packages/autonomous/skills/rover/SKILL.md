...
- "left for the operator's triage", "aan de operator om te triageren"
- "out of scope because it existed already"
- "someone else raised this, someone else closes it"

Every open item on the branch gets the same two fates as every pride finding: fixed, or rejected with concrete evidence of non-issue via pride's second-pass gate. "It was filed thirteen days ago" is not evidence of non-issue; it is a date. A reviewer whose concern is still valid on the current code is still valid, whether they filed it this morning or last quarter.

There is one honest exception: an item that literally does not live on this branch (a separate PR, an unrelated file, a concern about code the branch never touches). Those items are outside the branch, not inside-and-deferred, and do not belong in the loop file's Not done section at all. If in doubt: read the item, check whether the current branch could resolve it, and default to "inside".
...
**Mission Understanding (before DRIVE).** After Plan is written and before transitioning to DRIVE, the rover writes a short paragraph into the Log under a `[HH:MM] Mission Understanding:` header. The paragraph states, in prose and in the Dispatch's language, what the rover has understood the mission to be. Three concrete pieces, each named:

1. **What the deliverable is.** A single sentence naming the artefact the operator will open at stop time. If the deliverable is a branch, name the branch and what state it needs to be in; if a document, name the file; if a change to a running system, name the endpoint or feature.
2. **What already lives on the deliverable at start.** A one-to-three sentence survey of the branch's current state beyond the rover's own changes: open review threads (count and topic), failing CI jobs, lint baseline, inherited TODOs the rover can already see, uncommitted work. This is where authorship and temporal arguments get flushed out early: the rover writes down what is there, not who put it there.
3. **What "finished" looks like.** One sentence describing the stop state, phrased so the operator can audit it against the Dispatch. The framing of this sentence is the diagnostic: a narrow technical state ("CI is green", "tests pass", "the endpoint returns 200") is a different mission from a deliverable state ("the branch is ready for its next step", "the page is ready for users", "the release is shippable"). Narrow framings silently exclude inherited work on the deliverable; deliverable framings include it. The rover chooses the framing that matches the Dispatch's destination, not the framing that matches the rover's own commits. If the sentence cannot be written without hedging ("mostly", "except for the pre-existing X"), the understanding is wrong and the rover re-reads the Dispatch before continuing.

This is not an approval gate. The rover writes the paragraph, logs it, and keeps moving; it does not pause waiting for operator confirmation, and there is no "yes" channel to confirm on. The paragraph exists so the rover declares its framing out loud (catching its own scope-shrink before code changes start) and so the operator reading along early in the mission has a concrete thing to compare against the Dispatch. If the operator disagrees, they intervene via `## Input` or `/autonomous:stop`; if they do not, the rover continues.

...
2. Create the mission branch. Pick a kebab-case name from the loop-file name, no slashes, no prefixes, no rover or space-mission words (`fix-stale-cache`, `build-auth-page`, `investigate-slow-queries`), and run `git checkout -b <name>`. Any uncommitted changes in the working tree ride along onto the new branch, which is fine; step 3 handles them. No default-branch check, no "are we on the right branch" ceremony: the operator picked the starting point when they dispatched. Record the branch name in the loop file's `branch:` field in step 5. One case the rover cannot decide alone: no `.git` in cwd (or a repo with zero commits). Then pull step 5 forward as a stub and open a prelaunch question asking whether to `git init -b main` or set it up differently; wait for the answer or the five-minute fuse.
...
4. Ensure `.autonomous/.gitignore` exists. One bash line handles both the fresh and the already-there case: `mkdir -p .autonomous && [ -f .autonomous/.gitignore ] || echo '*' > .autonomous/.gitignore`. If the file is already there, this is a no-op; if it is missing, it lands with content `*`.

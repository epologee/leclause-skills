...
**Manually via `/autonomous:trim`:**
- `/autonomous:trim` reviews the uncommitted changes plus commits on the current branch not yet on the default branch
- `/autonomous:trim <ref>` reviews a specific commit range, for example `main..HEAD` or `HEAD~3..HEAD`
- `/autonomous:trim uncommitted` reviews only the uncommitted diff
...
**Invoked manually (`/autonomous:trim`):**

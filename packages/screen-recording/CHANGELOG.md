...
`/leclause:whats-new screen-recording` to re-read at any time.
...
- **Viewport-only is the principle, Playwright the example.** The "NEVER `screencapture`, ALWAYS Playwright" framing is replaced by "the outcome is viewport-only capture" with Playwright as the shipped implementation; sessions with another viewport-only route can use it.

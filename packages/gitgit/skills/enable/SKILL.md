...
user-invocable: true
description: >
  Use ONLY when the operator types `/gitgit:enable`. Do not auto-invoke.
  Re-enables the gitgit PreToolUse:Bash guards for the current Claude session
  by removing the sentinel file written by /gitgit:disable.
argument-hint: ""
---
...
de sentinel die `/gitgit:disable` heeft aangemaakt. Heeft geen effect als de
...
Gebruik `/gitgit:status` om te zien welke sentinels actief zijn en wat de
...
1. Bepaal de huidige session_id via dezelfde logica als `/gitgit:disable`:

#!/bin/bash
# packages/gitgit/hooks/guards/repo-deny.sh
# PreToolUse:Bash guard. Honours a per-repo lock set by /gitgit:disable-git.
# When the sentinel ".git/gitgit-deny" exists in the active repo, every git
...
#
# Bypass: /gitgit:enable-git removes the sentinel.
    ...
    if [[ -f "$dir/.git" ]]; then
      ...
      if [[ -n "$gitdir" ]]; then
        ...
        if [[ -f "$gitdir/commondir" ]]; then
          ...
          if [[ "$commondir" = /* ]]; then
            sentinel="$commondir/gitgit-deny"
          else
            sentinel="$gitdir/$commondir/gitgit-deny"
          fi
        else
          sentinel="$gitdir/gitgit-deny"
        fi
      fi
      break
    fi
...
"git lock active for this repo (.git/gitgit-deny).${reason_part} Subcommand '$subcmd' is a mutation. Run /gitgit:enable-git to lift, or use a read-only command (status, log, diff, show, rev-parse, blame)."
}

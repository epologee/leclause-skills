...
#   2. contains `allow-comment` (operator escape, one per comment, inline);
#   3. matches the pragma/directive allowlist (linter directives, compiler
#      magic comments, generated-file headers, copyright notices);
#   4. starts the first line with `!` (shebang).
#
...
dd_ncc_mode_for() {
  ...
  base=$(basename -- "$path")
  case "$path" in
    *.js|*.jsx|*.ts|*.tsx|*.mjs|*.cjs)        echo slash ;;
    *.swift)                                  echo slash ;;
    *.kt|*.kts)                               echo slash ;;
...
"Comment added in ${file}:${lineno}:${short}. Rewrite for clarity (intent-revealing name, extracted method, sharper signature) instead. Exceptions: comment with an http(s) URL, or 'allow-comment <reason>' inline escape."
  done <<< "$new_comments"
}
...
dd_ncc_is_allowed() {
  ...
  case "$body" in
    ...
    *"Copyright "*|*"SPDX-License-Identifier"*|*"License:"*)                              return 0 ;;
    *encoding:*|*coding:*)                                                                return 0 ;;
    go:*)                                                                                 return 0 ;;
  esac

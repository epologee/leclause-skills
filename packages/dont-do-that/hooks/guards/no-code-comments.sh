...
"Code comment introduced in ${file}:${lineno}:${short}. Rewrite for clarity (intent-revealing name, extracted method, sharper signature) instead. Exceptions: comment with an http(s) URL, or 'allow-comment: <reason>' inline escape."
  done <<< "$new_comments"
}

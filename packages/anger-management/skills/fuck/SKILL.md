...
name: fuck
user-invocable: true
description: Invoked as /fuck. Logs a one-line friction paper-cut for later review; runs only when the operator explicitly types this command, never auto-activated.
effort: low
---

# /fuck

The operator vented "fuck". This is a friction paper-cut, not a request to fix
...
1. Distil what grated into a plain one-line pointer of at most a dozen words, then
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/bin/anger-log" fuck <<'VENT_NOTE'
   <pointer>
   VENT_NOTE
   ```
...
2. Let the command's `logged fuck: ...` line stand as the acknowledgement, plus at

...
name: damn
user-invocable: true
description: Invoked as /damn. Logs a one-line friction paper-cut for later review; runs only when the operator explicitly types this command, never auto-activated.
effort: low
---

# /damn

The operator vented "damn". This is a friction paper-cut, not a request to fix
...
1. Distil what grated into a plain one-line pointer of at most a dozen words, then
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/bin/anger-log" damn <<'VENT_NOTE'
   <pointer>
   VENT_NOTE
   ```
...
2. Let the command's `logged damn: ...` line stand as the acknowledgement, plus at

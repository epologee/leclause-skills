...
`/leclause:whats-new recursion` to re-read at any time.
...
- **Sibling-skill references use suggestion-with-fallback.** The recursion orchestrator's approve action and the research sub-skill's plan-execution prompt name `/auto-loop` as default with an explicit fallback for sessions that lack it.
